...
def find_best_cluster_iloss(record, clusters):
    ...
    low=0
    high=len(clusters)-1
    min_index=getMin(low, high, record, clusters)
    return min_index

def getMin(l, h, record, clusters):
    """residual assignment. Find best cluster for record."""
    #min_index=0
    #min_distance = 1000000000000
    #best_cluster = clusters[0]
    ...
    if low == high:
        min_index =low
        distance = r_distance(record,clusters[low].gen_result)
        min_distance = distance
        best_cluster = clusters[low]
        return min_index
    elif high == low + 1:
        distancel = r_distance(record, clusters[low].gen_result)
        distanceh = r_distance(record, clusters[high].gen_result)
        if distancel > distanceh:
            min_index =high
            min_distance = distanceh
            best_cluster = clusters[high]
            return min_index
        else:
            min_index =low
            min_distance = distancel
            best_cluster = clusters[low]
            return min_index
    else:
        ...
        mid = int((low + high) / 2)
        min_index1 = getMin(low, mid, record, clusters)
        min_index2 = getMin(mid + 1, high, record, clusters)
 
    distancemin1 = r_distance(record, clusters[min_index1].gen_result)
    distancemin2 = r_distance(record, clusters[min_index2].gen_result)
    ...
    return min_index
def getMin_i(l, h, record, clusters):
    """residual assignment. Find best cluster for record."""
    #min_index=0
    #min_distance = 1000000000000
    #best_cluster = clusters[0]
    ...
    if low == high:
        min_index =low
        distance = diff_distance(record, clusters[low])
        min_distance = distance
        best_cluster = clusters[low]
        return min_index
    elif high == low + 1:
        distancel = diff_distance(record, clusters[low])
        distanceh = diff_distance(record, clusters[high])
        if distancel > distanceh:
            min_index =high
            min_distance = distanceh
            best_cluster = clusters[high]
    ...
    distancemin1 = diff_distance(record, clusters[min_index1])
    distancemin2 = diff_distance(record, clusters[min_index2])
    if(distancemin1<distancemin2):
        return min_index1
    else:
        return min_index2
...
def find_best_cluster_iloss_increase(record, clusters):
    ...
    low=0
    high=len(clusters)-1
    min_index=getMin_i(low, high, record, clusters)
    return min_index
...
def find_best_record_iloss_increase(cluster, data):
    ...
    low=0
    high=len(data)-1
    min_index=getMinr(low, high,cluster,data)
    return min_index
...
def getMinr(l, h,cluster,data):
    ...
    if low == high:
        min_index =low
        distance = diff_distance(data[low], cluster)
        min_distance = distance
        #best_cluster = low
        return min_index
    elif high == low + 1:
        distancel = diff_distance(data[low], cluster)
        distanceh = diff_distance(data[high], cluster)
        if distancel > distanceh:
            min_index =high
            min_distance = distanceh
            #best_cluster = high
        ...
        # If there are more than 2 elements
        mid = int((low + high) / 2)
        min_index1 = getMinr(low, mid, cluster, data)
        min_index2 = getMinr(mid + 1, high, cluster, data)
 
    distancemin1 = diff_distance(data[min_index1], cluster)
    distancemin2 = diff_distance(data[min_index2], cluster)
    if(distancemin1<distancemin2):
        return min_index1
    else:
        return min_index2
